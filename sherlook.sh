...
# NOTE (bugfix): this used to expand NODES with every ISO-3166 country code
# (~250 entries), regardless of whether Tor actually has exit relays there.
# That's what made the location list blow up / render empty in some views —
# far beyond the ~80 countries Tor realistically supports, and mostly full of
# countries with zero exit relays. The static NODES table above already
# tracks the real, commonly-available Tor exit countries, so we no longer
# expand it. Availability for any given country is still verified live
# (via Onionoo) at the moment you actually deploy it — see
# check_country_exit_availability() / deploy_node().
...
# NOTE (bugfix): this used to grow NODES/ORDER on every call by merging in
# whatever countries a live Onionoo fetch happened to report at that exact
# moment. If that fetch failed, was slow, or Onionoo returned a short list,
# the on-screen location menu would shrink or even show ZERO entries (the
# "Available Tor Exit Locations... [00] Back" bug with nothing above it).
# The location list you SEE is now always the static, fixed NODES table
# above — it never depends on network access. Live Onionoo data is only
# ever used to verify a specific country actually has exit relays right
# before we deploy it (see check_country_exit_availability), never to
# decide what's shown in the menu.
...
check_ip_quality() {
    ...
    # NOTE (bugfix): "all 3 GeoIP APIs were unreachable" used to be treated
    # as bad=1 (reject), which is wrong — Tor's own ExitNodes {CC} +
    # StrictNodes 1 already guarantees the exit is in the requested country;
    # a GeoIP lookup failure just means our own network to those APIs is
    # flaky. Rejecting on that caused good exits to be discarded and nodes
    # to loop through retries/rebuilds forever. We now only reject on a
    # confirmed mismatch or confirmed high abuse score.
    local bad=0 reason=""
...
# NOTE (bugfix): the "--install" CLI flag used to be handled right here,
# but install_engine() isn't defined until much further down in this file —
# calling it this early raised "install_engine: command not found" any time
# someone ran `sherlook --install` directly. The check now lives at the
# bottom of the file, after every function has been defined.
...
rotate_one_node_core() {
    ...
    # NOTE (bugfix): a dead process can't respond to a NEWNYM control-port
    # signal at all, so the old code below would burn through
    # NODE_ROTATE_RETRIES attempts of "still has no valid IP" against a
    # process that simply wasn't running, before ever reaching the restart
    # step. If the daemon is down, restart it immediately instead of
    # waiting through a full useless retry loop first.
    if ! node_process_running "$code" "$out_port"; then
